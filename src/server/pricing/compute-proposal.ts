...
export interface RotationLineState {
  ...
  truckFee: number;
  /** Optional display / catalog hint (not used by rotation $ math). */
  plantName?: string;
}
...
export function computeRotationMonthly(
  ...
  const p2 =
    (((qty / config.rotationPlantsPerHour) * f) / 12) * config.hourlyRate;
...
export function computeProposal(
    ...
    if (item.category === "plant") {
      ...
      if (item.plantingWithoutPot) {
        totals.materials.retail +=
          ...
          config.plantingWithoutPotFeePerPlant;
      }
      if (item.guaranteed) {
  ...
  for (const line of input.laborLines) {
    const v = line.people * line.hours * config.hourlyRate;
  ...
  const overhead = (costPerMonthHours + costPerMonthPlants) * overheadFactor;
