...
export function SidebarNav({ collapsed }: { collapsed: boolean }) {
  ...
  return (
    <nav
      className={`relative flex h-full flex-col border-r border-slate-800/80 bg-[#0c1222] ${
        collapsed ? "w-[72px]" : "w-64"
      }`}
    >
      <div
        className={`flex shrink-0 items-center border-b border-slate-800/80 px-3 py-4 ${
      ...
      <div className="relative z-20 border-b border-slate-800/80 p-3" ref={deptMenuRef}>
        {!collapsed ? (
          <>
            ...
            <div className="relative">
              <button
                ...
                className="flex w-full items-center justify-between gap-2 rounded-lg border border-slate-700/80 bg-slate-900/60 px-3 py-2.5 text-left text-sm text-white transition hover:border-slate-600 hover:bg-slate-800/80"
              ...
              >
                ...
                <ChevronDown
                  className={`h-4 w-4 shrink-0 text-slate-400 transition ${deptMenuOpen ? "rotate-180" : ""}`}
                />
              </button>
              {deptMenuOpen ? (
                <ul
                  className="absolute left-0 right-0 z-50 mt-1 max-h-64 overflow-y-auto rounded-lg border border-slate-700 bg-[#111827] py-1 shadow-xl"
                  role="listbox"
                >
                  {SIDEBAR_DEPARTMENT_OPTIONS.map((opt) => (
                    <li key={opt.id} role="presentation">
                      <button
                        ...
                        className={`flex w-full flex-col items-start gap-0.5 px-3 py-2.5 text-left text-sm transition hover:bg-white/5 ${
                          opt.id === selectedDeptId ? "bg-emerald-600/20 text-emerald-200" : "text-slate-200"
                        }`}
                      >
                        <span className="font-medium">{opt.name}</span>
                        <span className="text-[11px] leading-snug text-slate-500">{opt.description}</span>
                      </button>
                    </li>
                  ))}
                </ul>
          ...
          <div className="relative flex justify-center">
            <button
              type="button"
              onClick={() => setDeptMenuOpen((o) => !o)}
              className="flex h-10 w-10 items-center justify-center rounded-lg border border-slate-700/80 bg-slate-900/60 text-slate-300 hover:bg-slate-800"
            ...
            {deptMenuOpen ? (
              <ul
                className="absolute left-full top-0 z-50 ml-1 max-h-72 w-64 overflow-y-auto rounded-lg border border-slate-700 bg-[#111827] py-1 shadow-xl"
                role="listbox"
              >
                {SIDEBAR_DEPARTMENT_OPTIONS.map((opt) => (
                  <li key={opt.id} role="presentation">
      ...
      <div
        className={`shrink-0 border-t border-slate-800/80 p-3 ${
          collapsed ? "flex justify-center" : ""
        }`}
      >
        <div
          className={`flex items-center gap-2 rounded-lg bg-slate-900/50 px-2 py-2 ring-1 ring-slate-800/80 ${
            collapsed ? "justify-center" : ""
          }`}
        >
          ...
          {!collapsed ? (
            <div className="min-w-0">
              ...
              <p className="truncate text-[10px] text-slate-500">General Manager</p>
            </div>
          ) : null}
        </div>
      </div>
