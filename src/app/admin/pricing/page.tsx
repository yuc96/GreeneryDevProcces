...
const PRICING_CONFIG_TABS: {
...
}[] = [
  {
    ...
    description:
      "Wholesale freight %, markups, hourly rate, rotation defaults.",
  },
  {
    id: "install",
...
function AdminPricingPageInner() {
  ...
  const [cfg, setCfg] = useState<PricingEngineConfig>(
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
              Use the tabs below to focus one area at a time. Values are saved
              to{" "}
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <label className="text-sm">
                  ...
                  <span className="mt-1 block text-[11px] text-gray-500 dark:text-gray-400">
                    Same step as markup range ({cfg.markupMin}–{cfg.markupMax},
                    step {cfg.markupStep}).
                  </span>
                </label>
                <label className="text-sm">
                ...
                <div className="text-sm sm:col-span-2">
                                ...
                                onChange={(e) =>
                                  setCfg((c) => ({
                                    ...
                                    rotationTruckFeeRanges:
                                      c.rotationTruckFeeRanges.map((x, i) =>
                                        i === idx
                                          ? {
                                              ...x,
                                              from: Math.max(
                                  ...
                                  setCfg((c) => ({
                                    ...
                                    rotationTruckFeeRanges:
                                      c.rotationTruckFeeRanges.length <= 1
                                        ...
                                        : c.rotationTruckFeeRanges.filter(
                                            (_, i) => i !== idx,
                                          ),
                                  }))
                                }
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-2">
                    <button
                      ...
                      onClick={() =>
                        setCfg((c) => {
                          const sorted = [...c.rotationTruckFeeRanges].sort(
                            (a, b) => a.from - b.from,
                          );
                          const last = sorted[sorted.length - 1];
                          const from =
                            last?.to == null ? last.from + 1 : last.to + 1;
                          const fee =
                            last?.fee ?? c.defaultRotationTruckFee ?? 50;
          ...
          >
            <section className="rounded-xl border border-emerald-200/80 bg-white p-5 shadow-sm dark:border-emerald-900/40 dark:bg-gray-900">
              ...
              <p className="mb-4 text-xs text-gray-500 dark:text-gray-400">
                ...
                (separate from the main <strong>Save</strong>, which only
                updates{" "}
                <code className="rounded bg-gray-100 px-1 dark:bg-gray-800">
              ...
              <div className="space-y-6 border-t border-gray-100 px-5 pb-5 pt-2 dark:border-gray-800">
                <section className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                  ...
                  <p className="mb-4 text-xs text-gray-500 dark:text-gray-400">
                    ...
                    interpolates linearly between points. Missing diameter uses
                    fallback threshold. Install workload for labor line
                    ...
                    <div className="mt-3 grid gap-3 sm:grid-cols-2">
                      ...
                      <label className="text-sm">
                        ...
                        <input
                          ...
                          className="mt-1 w-full rounded border border-gray-200 px-2 py-1 dark:border-gray-700 dark:bg-gray-950"
                          value={
                            cfg.laborAuto.missingDiameterFallbackMinEmployees
                          }
                          onChange={(e) =>
                            setCfg((c) => ({
                              ...c,
                  ...
                  <div className="grid gap-4 sm:grid-cols-3">
                    ...
                    <label className="text-sm">
                      ...
                      />
                      <span className="mt-1 block text-[11px] text-gray-500 dark:text-gray-400">
                        Maximum clock minutes per person. Use 120 for a 2-hour
                        cap; when workload exceeds this target, the engine adds
                        people in parallel.
                      </span>
                    </label>
                    <label className="text-sm">
                  ...
                  <div className="mt-6 border-t border-gray-100 pt-4 dark:border-gray-800">
                    ...
                    <p className="mb-3 text-xs text-gray-500 dark:text-gray-400">
                      When the proposal includes plants, each phase is billed at
                      least this many person-minutes (even for a single plant
                      with one installer), before adding the plant-derived
                      portion split by the weights below.
                    </p>
                    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                      {(["load", "unload", "install", "cleanUp"] as const).map(
                        (key) => (
                          <label key={key} className="text-sm">
                            ...
                            <input
                              ...
                              className="mt-1 w-full rounded border border-gray-200 px-2 py-1 dark:border-gray-700 dark:bg-gray-950"
                              value={
                                cfg.laborAuto.handlingMinimumPersonMinutes[key]
                              }
                              onChange={(e) =>
                                setCfg((c) => ({
                                  ...c,
                ...
                <section className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                  ...
                  <p className="mb-3 text-xs text-gray-500 dark:text-gray-400">
                    ...
                    staffing results). They do not need to add up to 100%: the
                    engine normalizes them to sum to 1. Drive lines ignore this
                    table and use route / fallback drive hours.
                  </p>
        ...
        {activeTab === "staging" ? (
          ...
          >
            <section className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
              ...
              <p className="mb-4 text-xs text-gray-500 dark:text-gray-400">
                Rules for auto-generating staging materials per plant. The
                engine picks a recipe using{" "}
                <strong>(complexity band, indoor/outdoor)</strong> and
                multiplies <code>qtyPerPlant × plant quantity</code> (rounded
                up). Each material is referenced by its <code>sourceId</code> in{" "}
                <code>src/data/staggings-list.json</code>. Remove components
                with qty=0 if you do not want to include a material.
              </p>
