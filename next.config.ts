 ...
 * or `ChunkLoadError` / failed dynamic imports in the browser, the build output is stale or torn.
 * Stop the server, run `bun run clean`, then `bun run dev` (or `bun run dev:fresh` in one step).
 * Do not switch Turbopack ↔ webpack without cleaning. Avoid saving this file while `next dev` is running.
 */
